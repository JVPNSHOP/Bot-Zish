...
# Full fixed installer for ZIVPN + Telegram bot
# Fix: send user-card messages with context.bot.send_message so Renew/Edit/Delete inline buttons always appear.
set -euo pipefail
IFS=$'\n\t'
...
echo "== ZIVPN + Public Telegram Bot Installer (fixed, ensure buttons on user cards) =="
...
# Bot script (public, ensure buttons are properly attached to user-card messages)
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (user edit/renew/delete) - fixed sending method so buttons appear
...
# States
STATE_USERNAME, STATE_PASSWORD, STATE_EXPIRY, STATE_RENEW, STATE_EDIT_PASS, STATE_EDIT_EXP = range(6)
...
# blinking symbols using emojis
def blink_symbols_for_status(status:str):
    # returns (on_symbol, off_symbol, label)
    if status == "online":
        return ("🟢", "⚪", "Online")
    if status == "expiring":
        return ("🟡", "⚪", "Expiring")
...
async def blink_message(app, chat_id:int, message_id:int, original_text:str, status:str, cycles:int=6, delay:float=0.6):
    on_sym, off_sym, label = blink_symbols_for_status(status)
    try:
        text_on = original_text.replace("{STATUS_ON}", on_sym)
        text_off = original_text.replace("{STATUS_ON}", off_sym)
        for i in range(cycles):
            try:
                if i % 2 == 0:
                    await app.bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=text_on, parse_mode=constants.ParseMode.MARKDOWN)
                else:
                    await app.bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=text_off, parse_mode=constants.ParseMode.MARKDOWN)
            except Exception:
                pass
        ...
        logger.debug("blink_message failed: %s", e)
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "menu_list":
        ...
        if not rows:
            ...
            return
        # display cards (no CPU/RAM/STORAGE in each card)
        ...
        for r in rows[:100]:
            ...
            if revoked:
                ...
                if online and left>0:
                    ...
                    status = "offline"
            on_sym, off_sym, status_label = blink_symbols_for_status("online" if status=="online" else ("expiring" if status=="expiring" else "offline"))
            left_text = f"{left} days left"
            if left<=0:
                status_label = "Expired"
            # User Card WITHOUT CPU/RAM/STORAGE, with Renew/Edit/Delete buttons
            body_template = (f"👤 User Card\n\n"
                    f"🏷️ Username: `{username}`\n"
                    f"🔑 Password: `{password}`\n"
                    f"📡 Server IP: `{ZIVPN_SERVER_IP}`\n"
                    f"⏳ Expires: `{expires_at}` ({left_text})\n"
                    f"Status: {{STATUS_ON}} {status_label}")
            ...
            try:
                # IMPORTANT: use bot.send_message (not q.message.reply_text) to ensure inline keyboard is attached reliably
                sent = await context.bot.send_message(chat_id=chat_id, text=body_template.replace("{STATUS_ON}", on_sym),
                                                     parse_mode=constants.ParseMode.MARKDOWN, reply_markup=kb_card)
                # schedule blinking edits on that sent message
                if revoked:
                    blink_status = "offline"
                else:
                    if user_online(password) and left>0:
                        blink_status = "online"
                    elif left<=3 and left>0:
                        blink_status = "expiring"
                    else:
                        blink_status = "offline"
                asyncio.create_task(blink_message(context.application, sent.chat_id, sent.message_id, body_template, blink_status))
            except Exception as e:
                logger.debug("failed sending card with buttons: %s", e)
                # fallback: reply to the callback message (may not attach buttons properly in some environments)
                try:
                    await q.message.reply_text(body_template.replace("{STATUS_ON}", on_sym))
                except Exception:
                    pass
        # edit original menu message to acknowledge display
        try:
...
async def action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "act_delete":
        ...
        try:
            ...
            total_users = cur.fetchone()[0] or 0
            conn.close()
            await q.edit_message_text(f"🗑️ User `{username}` revoked and removed from config.\nTotal Users: `{total_users}`", parse_mode=constants.ParseMode.MARKDOWN)
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(edit_pass_callback, pattern='^edit_pass\\|'))
    app.add_handler(CallbackQueryHandler(edit_exp_callback, pattern='^edit_exp\\|'))
