#!/usr/bin/env bash
# install_zivpn_public_fixed_v6.9.1.sh
# Enhanced installer for ZIVPN + Telegram bot (v6.9.1) with Button-Based Admin Commands
# - COMPLETELY FIXED: Restore function now works properly
# - FIXED: Button interface for all commands
# - FIXED: File upload handling in restore
# - COMPLETELY FIXED: Password addition to ZIVPN config
# - NEW: Button-based interface for all admin commands
# - NEW: /start shows buttons for all functions
# - Enhanced UI with icons for all buttons
...
echo "== ZIVPN + Public Telegram Bot Installer (v6.9.1 - Fixed Restore) =="
...
# Restore script - COMPLETELY FIXED VERSION v6.9.1
sudo tee /usr/local/bin/zivpn_restore.py > /dev/null <<'PY'
#!/usr/bin/env python3
# ZIVPN Restore Script for Telegram Bot - COMPLETELY FIXED VERSION v6.9.1
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("Example: zivpn_restore.py /tmp/backup.tar.gz YOUR_BOT_TOKEN 123456789")
        sys.exit(1)
...
# ============================================================================
# Bot script (v6.9.1) - COMPLETELY FIXED with Button-Based Interface and RESTORE FIXED
# ============================================================================
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (fixed v6.9.1) with BUTTON-BASED Interface
# COMPLETELY FIXED VERSION - Button interface for all commands + RESTORE FIXED
import os, sqlite3, subprocess, datetime, json, logging, asyncio, shutil, math, secrets, string, tempfile, tarfile, io, sys, threading
import psutil, requests, hashlib, re
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("/var/log/zivpn_bot.log"),
...
logger.info("Started zivpn public bot (fixed v6.9.1 with Button Interface) admin_notify=%s", ADMIN_IDS)
if not BOT_TOKEN:
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, message: str = None):
    ...
    keyboard.append([KeyboardButton("🆕 Create Account"), KeyboardButton("👥 Users List")])
    keyboard.append([KeyboardButton("🔍 Search Users"), KeyboardButton("ℹ️ Help Info")])
    
    # Admin buttons (only for admins)
    if update.effective_user.id in ADMIN_IDS:
        keyboard.append([KeyboardButton("⚙️ Admin Panel"), KeyboardButton("📊 System Stats")])
...
async def show_admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [KeyboardButton("📦 Create Backup"), KeyboardButton("📥 Restore Backup")],
        [KeyboardButton("🔄 Restart Services"), KeyboardButton("🔍 Check Status")],
        [KeyboardButton("🔧 Rebuild Config"), KeyboardButton("📊 System Stats")],
        [KeyboardButton("🗑️ Uninstall ZIVPN"), KeyboardButton("🔙 Main Menu")]
    ]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=False)
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Start command with button interface"""
    total = total_users_count()
    stats = get_server_stats()
    
    welcome_text = (
        f"🤖 *ZIVPN Management Bot v6.9.1*\n\n"
        f"📊 *System Status:*\n"
        f"├ Users: `{total}`\n"
        f"├ CPU: {stats['cpu']:.1f}%\n"
        f"├ RAM: {stats['ram_percent']}%\n"
        f"└ Storage: {stats['disk_percent']}%\n\n"
        f"📡 *Server Info:*\n"
        f"├ IP: `{ZIVPN_SERVER_IP}`\n"
    )
    
    if ZIVPN_DOMAIN:
        welcome_text += f"└ Domain: `{ZIVPN_DOMAIN}`\n\n"
    else:
        welcome_text += "\n"
    
    welcome_text += "🎛️ *Use buttons below to manage your VPN:*"
...
async def handle_main_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"Button pressed: {txt} by user {user_id}")
    ...
    if txt == "🆕 Create Account":
    ...
    elif txt == "🔍 Search Users":
        context.user_data['awaiting_search'] = True
        await update.message.reply_text(
            "🔍 *Search Users*\n"
            "Send username to search (partial matches allowed).",
            parse_mode=constants.ParseMode.MARKDOWN
        )
    
    elif txt == "ℹ️ Help Info":
        help_text = (
            "📚 *ZIVPN Bot Help*\n\n"
            "🆕 *Create Account:* Create new VPN account\n"
            "👥 *Users List:* View all active accounts\n"
            "🔍 *Search Users:* Search for specific account\n"
            "🔁 *Renew:* Extend account expiry (from user list)\n"
            "✏️ *Edit:* Change password or expiry (from user list)\n"
            "🗑️ *Delete:* Revoke account access (from user list)\n\n"
        ...
        if user_id in ADMIN_IDS:
            ...
            help_text += "📦 Create Backup - Backup system\n"
            help_text += "📥 Restore Backup - Restore from backup\n"
            help_text += "🔄 Restart Services - Restart ZIVPN\n"
            help_text += "🔍 Check Status - Check ZIVPN status\n"
            help_text += "🔧 Rebuild Config - Fix configuration\n"
            help_text += "📊 System Stats - Detailed statistics\n"
            help_text += "🗑️ Uninstall ZIVPN - Remove system\n"
        ...
        if user_id in ADMIN_IDS:
            ...
            await update.message.reply_text("🚫 This panel is for admins only.")
    
    elif txt == "📊 System Stats":
        await cmd_stats(update, context)
    
    else:
        # Fallback to text router for other inputs
...
async def handle_admin_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"Admin button pressed: {txt} by user {user_id}")
    
    if txt == "📦 Create Backup":
        await cmd_backup(update, context)
    
    elif txt == "📥 Restore Backup":
        await cmd_restore(update, context)
    
    elif txt == "🔄 Restart Services":
        ...
        await cmd_check_zivpn(update, context)
    
    elif txt == "🔧 Rebuild Config":
        await cmd_rebuild_config(update, context)
    
    elif txt == "📊 System Stats":
        await cmd_stats(update, context)
    
    elif txt == "🗑️ Uninstall ZIVPN":
        await cmd_uninstall(update, context)
    
    elif txt == "🔙 Main Menu":
        await show_main_menu(update, context, "Returning to main menu...")

# ========== ADMIN COMMANDS (Button-triggered) ==========
@admin_only
...
async def cmd_check_zivpn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if auth_passwords_count != db_active_users:
            ...
            status_text += "Use '🔧 Rebuild Config' to fix this.\n"
        
        if service_status != "active":
            status_text += f"⚠️ **WARNING:** ZIVPN service is not active!\n"
            status_text += "Use '🔄 Restart Services' to restart.\n"
    ...
    try:
        ...
        def rebuild_config():
            ...
            try:
                ...
                return True, f"Config rebuilt with {len(active_passwords)} passwords"
                
            except Exception as e:
                return False, str(e)
        
        # Run rebuild
        success, message = rebuild_config()
...
async def global_text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt in ("🆕 Create Account", "👥 Users List", "🔍 Search Users", "ℹ️ Help Info", 
               "⚙️ Admin Panel", "📊 System Stats"):
        return
    
    # Skip if it's an admin button
    if txt in ("📦 Create Backup", "📥 Restore Backup", "🔄 Restart Services", 
               "🔍 Check Status", "🔧 Rebuild Config", "🗑️ Uninstall ZIVPN", "🔙 Main Menu"):
        return
    ...
    # If no handler matched, show help
    await update.message.reply_text(
        "🤔 Not sure what you want. Use the buttons below or type:\n"
...
# ========== FIXED BACKUP/RESTORE FILE HANDLING ==========
async def handle_backup_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle backup file upload for restore - FIXED VERSION"""
    try:
        ...
        file_name = update.message.document.file_name or "backup.tar.gz"
        ...
        temp_dir = tempfile.mkdtemp(prefix="zivpn_restore_")
        file_path = os.path.join(temp_dir, file_name)
        
        logger.info(f"Downloading backup file: {file_name} to {file_path}")
        ...
        try:
            ...
            logger.info(f"Backup file downloaded: {file_path}, size: {file_size_mb:.2f} MB")
            
            # Create a unique ID for this file to avoid callback data issues
            import uuid
            file_id = str(uuid.uuid4())[:8]
            
            # Store file info in user_data
            context.user_data['restore_file_id'] = file_id
            context.user_data[f'restore_file_{file_id}'] = file_path
            context.user_data[f'restore_temp_dir_{file_id}'] = temp_dir
            
            # Ask for confirmation
            keyboard = [
                [InlineKeyboardButton("✅ Yes, Restore", callback_data=f"restore_yes_{file_id}")],
                [InlineKeyboardButton("❌ Cancel", callback_data=f"restore_no_{file_id}")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            ...
            logger.info(f"Waiting for restore confirmation for file: {file_path}")
        ...
        logger.error(f"Error in handle_backup_file: {e}", exc_info=True)
        if update.message:
            await update.message.reply_text(f"❌ Error processing file: {str(e)}")
        return STATE_WAITING_BACKUP_FILE

async def handle_restore_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle restore confirmation callback - FIXED"""
    query = update.callback_query
    await query.answer()
    
    # Check if this is a cancel request
    if query.data.startswith("restore_no_"):
        file_id = query.data.replace("restore_no_", "")
        
        # Cleanup temp files
        file_path = context.user_data.get(f'restore_file_{file_id}')
        temp_dir = context.user_data.get(f'restore_temp_dir_{file_id}')
        ...
        # Clean up user data
        for key in ['restore_file_id', f'restore_file_{file_id}', f'restore_temp_dir_{file_id}']:
    ...
    # Handle restore confirmation
    elif query.data.startswith("restore_yes_"):
        file_id = query.data.replace("restore_yes_", "")
        
        # Get file path from user_data
        file_path = context.user_data.get(f'restore_file_{file_id}')
        temp_dir = context.user_data.get(f'restore_temp_dir_{file_id}')
        
        if not file_path or not os.path.exists(file_path):
            await query.edit_message_text("❌ Backup file not found. Please try the restore process again.")
            return ConversationHandler.END
        
        await query.edit_message_text("🔄 Restoring from backup... This may take 1-2 minutes.")
        ...
        def run_restore():
            try:
                ...
                # Run restore script
                cmd = ["sudo", "/usr/local/bin/zivpn_restore.py", file_path, bot_token, admin_id]
                ...
                # Log result
                if result.returncode != 0:
                    logger.error(f"Restore script failed: {result.stderr}")
                else:
                    logger.info("Restore completed successfully")
                
                # Clean up user data
                context.user_data.pop(f'restore_file_{file_id}', None)
                context.user_data.pop(f'restore_temp_dir_{file_id}', None)
                ...
                try:
                    ...
                    data = {
                        ...
                        'text': "❌ Restore timed out. Please try again.",
                        'parse_mode': 'HTML'
                    }
...
def build_conv():
    conv = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(handle_restore_callback, pattern='^(restore_yes_|restore_no_)')
        ],
        states={
...
def main():
    ...
    # Button handlers
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex('^(🆕 Create Account|👥 Users List|🔍 Search Users|ℹ️ Help Info)$'), handle_main_buttons))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex('^(📦 Create Backup|📥 Restore Backup|🔄 Restart Services|🔍 Check Status|🔧 Rebuild Config|📊 System Stats|🗑️ Uninstall ZIVPN|🔙 Main Menu)$'), handle_admin_buttons))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex('^(⚙️ Admin Panel|📊 System Stats)$'), handle_main_buttons))
    
    # Global text router for all other text messages
    ...
    logger.info("Public bot polling started (fixed v6.9.1 with Button Interface)")
    app.run_polling()
...
[Unit]
Description=ZIVPN Telegram Bot (PUBLIC MODE - Button Interface)
After=network.target
...
echo ""
echo "== INSTALL COMPLETE v6.9.1 with BUTTON-BASED INTERFACE and FIXED RESTORE =="
echo ""
echo "✅✅✅ COMPLETELY FIXED: Restore function now works properly"
echo "✅✅✅ BUTTON-BASED INTERFACE: All commands available via buttons"
echo "✅✅✅ ENHANCED UI: Beautiful icons for all buttons"
echo ""
echo "🎛️ **NEW BUTTON INTERFACE:**"
echo "   /start နှိပ်ရင် button တွေပေါ်လာပါမယ်:"
echo "   • 🆕 Create Account - အသစ်ဖန်တီးရန်"
echo "   • 👥 Users List - လူစာရင်းကြည့်ရန်"
echo "   • 🔍 Search Users - ရှာဖွေရန်"
echo "   • ℹ️ Help Info - အကူအညီ"
echo ""
echo "   👑 **Admin Panel (Admins only):**"
echo "   • ⚙️ Admin Panel - Admin commands menu"
echo "   • 📊 System Stats - System statistics"
echo ""
echo "   ⚙️ **Admin Panel Buttons:**"
echo "   • 📦 Create Backup - Backup system"
echo "   • 📥 Restore Backup - Restore from backup"
echo "   • 🔄 Restart Services - Restart ZIVPN"
echo "   • 🔍 Check Status - Check ZIVPN status"
echo "   • 🔧 Rebuild Config - Fix configuration"
echo "   • 📊 System Stats - Detailed statistics"
echo "   • 🗑️ Uninstall ZIVPN - Remove system"
echo "   • 🔙 Main Menu - Back to main menu"
echo ""
echo "🔧 **RESTORE FUNCTION COMPLETELY FIXED:**"
echo "   - File upload now works properly"
echo "   - No more hanging during restore"
echo "   - Progress notifications will be sent"
echo "   - Automatic cleanup after restore"
echo ""
echo "✅ **User Card Format with DOMAIN:**"
echo "   👑 Premium User : 1"
echo "   📡 Server IP : xxxx"
...
echo "   - Rebuild config: Admin Panel → 🔧 Rebuild Config"
echo ""
...
echo ""
echo "✅ Installation complete with COMPLETELY FIXED RESTORE FUNCTION!"
