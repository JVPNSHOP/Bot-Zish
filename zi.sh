...
# Public ZIVPN + Telegram bot installer (fixed conversation; numeric expiry accepted)
# Added: interactive BOT_TOKEN prompt & Total Users Count shown in bot messages
set -euo pipefail
IFS=$'\n\t'

### ===== CONFIG (edit BEFORE running if needed) =====
BOT_TOKEN_DEFAULT=""   # leave empty to be prompted at install time
ADMIN_IDS_DEFAULT=""   # public mode = empty (comma separated Telegram IDs for admin notifications)
# detect public IP (fallback to placeholder)
ZIVPN_SERVER_IP_DEFAULT="$(curl -s https://ifconfig.co || echo "your.server.ip")"
### ==================================================

echo "== ZIVPN + Public Telegram Bot Installer (fixed) =="
echo "Running as: $(whoami) on $(hostname)"
sleep 1

# If BOT_TOKEN_DEFAULT empty, prompt user (useful when running from Termius)
if [ -z "${BOT_TOKEN_DEFAULT}" ]; then
  echo
  echo "Enter your Telegram BOT TOKEN (format: 123456:ABC-DEF... )."
  echo "If you leave blank, installation will continue but bot will not start until /etc/default/zivpn_bot is filled."
  read -r -p "BOT TOKEN (press Enter to skip): " BOT_TOKEN_INPUT
...
if [ -z "${ADMIN_IDS_DEFAULT}" ]; then
  echo
  read -r -p "Enter ADMIN_IDS (comma separated Telegram user IDs) or press Enter to leave empty: " ADMIN_IDS_INPUT
  if [ -n "$ADMIN_IDS_INPUT" ]; then
    ADMIN_IDS_DEFAULT="$ADMIN_IDS_INPUT"
  fi
fi

# Ask to confirm server IP or accept detected value
echo
read -r -p "Server IP to show in messages [${ZIVPN_SERVER_IP_DEFAULT}]: " SERVER_IP_INPUT
...
# Bot script (public, fixed to accept numeric expiry input + show total users)
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (fixed conversation; numeric expiry accepted)
...
async def confirm_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "confirm_add":
        ...
        try:
            ...
            conn.commit()
            # compute total users count (all rows)
            cur.execute("SELECT COUNT(*) FROM accounts")
...
echo "Bot is running? Check: sudo systemctl status zivpn_bot.service"
echo "If you did not provide BOT_TOKEN during install, open /etc/default/zivpn_bot and set BOT_TOKEN=\"<your token>\" then: sudo systemctl restart zivpn_bot.service"
echo "To test: open bot and send /start -> Create Account -> follow the flow."
echo "Added Total Users count in confirmation messages."
echo "To watch logs: sudo journalctl -u zivpn_bot.service -f"
echo "Done."
