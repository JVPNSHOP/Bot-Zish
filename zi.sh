#!/usr/bin/env bash
# install_zivpn_public_stable.sh
# Full stable installer for ZIVPN + Telegram bot (NO BLINK — buttons won't disappear)
set -euo pipefail
IFS=$'\n\t'
...
echo "== ZIVPN + Public Telegram Bot Installer (stable, no blink) =="
...
# Bot script (stable: NO blink edits; user cards use bot.send_message so buttons persist)
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (stable, no blink)
...
logger.info("Started zivpn public bot (stable) admin_notify=%s", ADMIN_IDS)
...
def main():
    ...
    logger.info("Public bot polling started (stable)")
    app.run_polling()
