...
# Fixed installer for ZIVPN + Telegram bot (v6.7) with Backup/Restore/Uninstall via Bot
...
# Restore script - COMPLETELY FIXED VERSION
...
def rebuild_zivpn_config_from_database():
    """Rebuild ZIVPN config.json from database - CRITICAL FIX"""
    db_path = "/var/lib/zivpn_bot/accounts.db"
    ...
    try:
        ...
        rows = cur.fetchall()
        active_passwords = [row[0] for row in rows if row[0]]
        ...
        print(f"🔧 Rebuilding ZIVPN config with {total_active} active passwords")
        ...
        cert_path = "/etc/zivpn/zivpn.crt"
        key_path = "/etc/zivpn/zivpn.key"
...
# Bot script (v6.7) - FIXED with better restore handling
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (fixed v6.7) with DOMAIN and Admin Commands
...
logger.info("Started zivpn public bot (fixed v6.7 with Admin Commands) admin_notify=%s", ADMIN_IDS)
if not BOT_TOKEN:
...
async def cmd_rebuild_config(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Import the rebuild function from restore script
        import importlib.util
        spec = importlib.util.spec_from_file_location("restore_module", RESTORE_SCRIPT)
        restore_module = importlib.util.module_from_spec(spec)
        
        # Read the restore script and extract the rebuild function
        with open(RESTORE_SCRIPT, 'r') as f:
            restore_code = f.read()
        
        # Execute the rebuild function
        exec_globals = {}
        exec(restore_code, exec_globals)
        
        if 'rebuild_zivpn_config_from_database' in exec_globals:
            success = exec_globals['rebuild_zivpn_config_from_database']()
            if success:
                ...
                await msg.edit_text("❌ Failed to rebuild ZIVPN configuration.")
        else:
            await msg.edit_text("❌ Rebuild function not found in restore script.")
...
def main():
    ...
    logger.info("Public bot polling started (fixed v6.7 with Admin Commands)")
    app.run_polling()
...
echo ""
echo "== INSTALL COMPLETE v6.7 with Telegram Bot Backup/Restore/Uninstall =="
echo ""
...
echo "✅ FIXED: Restore process rebuilds ZIVPN config from database"
echo "✅ FIXED: ZIVPN works after restore"
echo ""
echo "🔧 **CRITICAL FIXES in v6.7:**"
echo "   1. Restore script now rebuilds config.json from database"
echo "   2. Active passwords are extracted from DB and added to auth.config"
echo "   3. Certificates are regenerated if missing"
echo "   4. Config.json structure is properly rebuilt"
echo ""
...
echo ""
echo "🔧 **If ZIVPN still doesn't connect after restore:**"
echo "  1. Use /check_zivpn to see the status"
echo "  2. Use /rebuild_config to manually rebuild the config"
echo "  3. Use /restart to restart services"
echo ""
...
echo "   - Config.json rebuilt from database"
echo "   - ZIVPN works immediately after restore"
...
echo "6. All users and settings restored! ZIVPN works immediately."
echo ""
...
echo "4. Test /backup, /restore, etc."
echo ""
