...
# Full fixed installer for ZIVPN + Telegram bot (v6.5)
set -euo pipefail
IFS=$'\n\t'
...
echo "== ZIVPN + Public Telegram Bot Installer (fixed v6.5) =="
...
sudo apt-get install -y wget curl openssl python3 python3-venv python3-pip ufw iptables sqlite3 jq netcat-openbsd

# Detect default IPv4 interface (NETIF)
NETIF=$(ip -4 route get 1.1.1.1 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="dev"){print $(i+1); exit}}')
if [ -z "$NETIF" ]; then
  NETIF=$(ip -4 route | awk '/default/ {print $5; exit}')
fi
: ${NETIF:=eth0}
...
# Create default config.json or sanitize existing
...
if not isinstance(j['auth'].get('config',[]),list):
    j['auth']['config']=list(j['auth'].get('config',[]))
if changed:
...
if ! sudo test -f /etc/zivpn/zivpn.crt || ! sudo test -f /etc/zivpn/zivpn.key; then
...
# sysctl tweaks & enable ip forwarding persistently
...
# iptables: remove problematic rules and add correct DNAT -> 127.0.0.1:5667
echo "Cleaning old NAT rules..."
mapfile -t DNAT_LINES < <(sudo iptables -t nat -L PREROUTING --line-numbers -n | awk '/DNAT/ && /6000:19999/ {print $1}' || true)
if [ "${#DNAT_LINES[@]}" -gt 0 ]; then
  for (( idx=${#DNAT_LINES[@]}-1 ; idx>=0 ; idx-- )) ; do
    sudo iptables -t nat -D PREROUTING "${DNAT_LINES[idx]}" 2>/dev/null || true
  done
fi

# Now add correct DNAT using detected interface and explicit IPv4 destination
sudo iptables -t nat -A PREROUTING -i "$NETIF" -p udp --dport 6000:19999 -j DNAT --to-destination 127.0.0.1:5667 || true

# Ensure POSTROUTING MASQUERADE for replies through the external interface
sudo iptables -t nat -D POSTROUTING -o "$NETIF" -j MASQUERADE 2>/dev/null || true
sudo iptables -t nat -A POSTROUTING -o "$NETIF" -j MASQUERADE || true

# UFW allow (safe: will succeed even if ufw disabled)
...
print("PASSWORD:%s EXPIRES:%s" % (pw, "%s" % ("$EXPIRES")))
PY
...
# Write full bot.py (v6.5) - full handlers included
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (fixed v6.5)
...
logger.info("Started zivpn public bot (fixed v6.5) admin_notify=%s", ADMIN_IDS)
...
async def handle_main_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == "👥 Users List":
        ...
        return
    if txt == "ℹ️ Help":
        await update.message.reply_text("📚 Public ZIVPN Bot\n\n"
...
def main():
    ...
    logger.info("Public bot polling started (fixed v6.5)")
    app.run_polling()
