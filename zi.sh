...
# Fixed installer for ZIVPN + Telegram bot (v6.5) with Backup/Restore/Uninstall via Bot
...
#!/usr/bin/env python3
# ZIVPN Restore Script for Telegram Bot
import os, tarfile, tempfile, shutil, sqlite3, json, subprocess, sys
from pathlib import Path
...
def restore_backup(backup_file_path):
    ...
    try:
        ...
        if os.path.exists(db_backup):
            ...
            subprocess.run(["sudo", "chown", f"{current_user}:{current_user}", "/var/lib/zivpn_bot/accounts.db"], check=False)
...
# Bot script (v6.5) - UPDATED with DOMAIN and Admin Backup/Restore/Uninstall
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (fixed v6.5) with DOMAIN and Admin Commands
...
logger.info("Started zivpn public bot (fixed v6.5 with Admin Commands) admin_notify=%s", ADMIN_IDS)
if not BOT_TOKEN:
        ...
        try:
            ...
            if result.returncode == 0:
                await query.edit_message_text(
                    ...
                    "You may need to /start the bot again."
                )
            ...
            await query.edit_message_text("❌ Restore timed out.")
        except Exception as e:
...
def main():
    ...
    logger.info("Public bot polling started (fixed v6.5 with Admin Commands)")
    app.run_polling()
...
echo ""
echo "== INSTALL COMPLETE v6.5 with Telegram Bot Backup/Restore/Uninstall =="
echo ""
...
echo ""
echo "✅ Admin Restore Features via Telegram Bot:"
echo "   - /restore command starts restore process"
echo "   - Upload backup file to bot"
echo "   - Confirmation before overwriting"
echo "   - Restore to same or different server"
echo "   - Automatic services restart"
echo ""
