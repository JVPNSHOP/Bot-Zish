#!/usr/bin/env bash
# install_zivpn_full_with_ui.sh
# Full installer: ZIVPN UDP + Telegram Bot with user cards (days left, online status, Renew/Delete)
# NOTE: This script embeds the BOT token and ADMIN ID you provided earlier. Rotate token if public.
set -euo pipefail
IFS=$'\n\t'

### ===== EMBEDDED CREDENTIALS (edit if needed) =====
...
echo "== ZIVPN + Telegram Bot (UI) Installer =="
...
if ! sudo wget -q https://raw.githubusercontent.com/zahidbd2/udp-zivpn/main/config.json -O /etc/zivpn/config.json; then
  echo "-- Fallback config.json created --"
...
# 4) Helper create script (adds password to /etc/zivpn/config.json)
...
# 5) Python environment and bot files
echo "-- Setting up bot environment in /opt/zivpn_bot --"
...
# 7) Environment file with embedded token/admin
...
# 8) Bot script with UI features
echo "-- Writing bot script to /opt/zivpn_bot/bot.py --"
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - ZIVPN Telegram Bot with Menu, Create wizard, Card-style User List (Days left, Online status),
# and Renew/Delete actions with icons.
import os, sqlite3, subprocess, datetime, json, logging, shlex
...
def user_online(password:str)->bool:
    # define "online" as present in /etc/zivpn/config.json and not expired/revoked
    try:
...
def run_create_script(username:str, days:int, password:str=None):
    ...
    # parse output e.g. PASSWORD:abc EXPIRES:YYYY-MM-DD
    pwd, exp = None, None
    for token in out.split():
        if ":" in token:
            k,v = token.split(":",1)
            if k.lower()=="password": pwd = v
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "menu_list":
        ...
        if not rows:
            ...
            return
        # send one card per user (up to 50)
        for r in rows[:100]:
        ...
        return
    if q.data == "menu_help":
        await q.edit_message_text("Use *Create Account* to add users. In *User List* you can Renew or Delete accounts. Only ADMIN_IDS can manage accounts.", parse_mode=constants.ParseMode.MARKDOWN)
        return
...
async def expiry_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    return STATE_CONFIRM if 'STATE_CONFIRM' in globals() else STATE_RENEW  # just to satisfy states
...
# Action callbacks (renew/delete)
async def action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    data = q.data  # format act_<type>|<username>
    user = q.from_user
    if not is_admin(user.id):
        await q.edit_message_text("Unauthorized.")
        return
    ...
    if action == "act_delete":
        ...
        # prompt admin to choose days to extend
        kb = [
...
async def renew_do_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("SELECT expires_at, password FROM accounts WHERE username=? LIMIT 1", (username,))
    row = cur.fetchone()
    if not row:
        await q.edit_message_text("User not found.")
        conn.close()
        return
    expires_at, password = row
    try:
...
# Renew custom -> admin will send number; store state in user_data
async def renew_custom_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) != 2:
        ...
        return
    username = parts[1]
    # store pending renew username in user_data
...
async def recv_renew_days(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # perform renew (same logic as renew_do_callback)
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
...
# 9) Create systemd service for bot (ensure EnvironmentFile is present)
...
echo "1) Edit /etc/default/zivpn_bot if you want to change BOT_TOKEN, ADMIN_IDS, ZIVPN_SERVER_IP"
echo "   (Currently embedded token/admins provided during install.)"
echo "2) If you change /etc/default/zivpn_bot, run:"
echo "   sudo systemctl daemon-reload"
echo "   sudo systemctl restart zivpn_bot.service"
echo ""
echo "3) Test bot:"
echo "   - Send /start in Telegram to the bot user (must be the bot with the token above)."
echo "   - Use Menu -> Create Account -> follow the flow."
echo "   - Use Menu -> User List to see Card view; use Renew/Delete buttons."
echo ""
echo "4) Logs:"
echo "   sudo journalctl -u zivpn_bot.service -f"
echo "   sudo journalctl -u zivpn.service -f"
echo ""
echo "SECURITY: If token was publicly exposed, rotate it immediately via @BotFather."
echo "Done."
