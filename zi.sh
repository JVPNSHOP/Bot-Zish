...
# Status line uses text-only indicators [G]/[Y]/[R]; other UI lines contain emojis.
set -euo pipefail
IFS=$'\n\t'
...
# Bot script (public, server stats + blink A) - updated: status text-only indicators; other places include emojis
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (server stats + blinking status A) - updated (emojis except status)
import os, sqlite3, subprocess, datetime, json, logging, asyncio, shutil, math
import psutil
...
def get_server_stats():
    """Return detailed server stats: cpu_percent, ram_used_mb, ram_total_mb, ram_percent, disk_used_gb, disk_total_gb, disk_percent"""
    try:
...
# blinking style: no emoji, alternate textual indicator
...
async def blink_message(app, chat_id:int, message_id:int, original_text:str, status:str, cycles:int=6, delay:float=0.6):
    """Edit the message alternately replacing the status indicator to simulate blinking.
       original_text should contain a placeholder string {STATUS_ON} where the on-symbol goes,
       and we'll replace it with {STATUS_OFF} during off cycles.
    """
    on_sym, off_sym, label = blink_symbols_for_status(status)
    try:
        # Build on/off versions
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "menu_list":
        ...
        for r in rows[:100]:
            ...
            # get latest server stats for each card
            stats = get_server_stats()
            cpu_line = f"⚙️ CPU: {stats['cpu']:.1f}%"
            ram_line = f"🧠 RAM: {stats['ram_used_mb']}MB/{stats['ram_total_mb']}MB ({stats['ram_percent']}%)"
            disk_line = f"💾 STORAGE: {stats['disk_used_gb']}GB/{stats['disk_total_gb']}GB ({stats['disk_percent']}%)"
            ...
            body_template = (f"👤 User Card\n\n"
                    ...
                    f"{cpu_line}\n{ram_line}\n{disk_line}")
            try:
                sent = await q.message.reply_text(body_template.replace("{STATUS_ON}", on_sym), parse_mode=constants.ParseMode.MARKDOWN,
                ...
                # schedule blinking edits (it will edit message several times to simulate blink)
                # determine blink status to pass
                if revoked:
                    ...
                    if user_online(password) and left>0:
                        ...
                        blink_status = "offline"
                # run blink task (it will edit message several times to simulate blink)
                asyncio.create_task(blink_message(context.application, sent.chat_id, sent.message_id, body_template, blink_status))
            except Exception:
                # fallback: one-time send without blink if fails
                try:
                    await q.message.reply_text(body_template.replace("{STATUS_ON}", on_sym), parse_mode=constants.ParseMode.MARKDOWN)
                except:
                    pass
