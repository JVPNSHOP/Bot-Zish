...
# - Renew custom days works
# - Delete removes card (and marks revoked)
# - Edit password/expiry works (conversation entry points fixed)
# - No blinking edits that remove buttons
set -euo pipefail
IFS=$'\n\t'
...
echo "== ZIVPN + Public Telegram Bot Installer (fixed v2) =="
...
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (fixed v2)
...
logger.info("Started zivpn public bot (fixed v2) admin_notify=%s", ADMIN_IDS)
...
async def action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "act_delete":
        ...
        try:
            ...
            total_users = cur.fetchone()[0] or 0
            conn.close()
            # delete the card message so it disappears from chat
            try:
                await q.message.delete()
            except Exception:
                pass
            # ack in a reply to the callback origin (best-effort)
            try:
...
async def renew_custom_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Start conversation for custom renew days (so the next text message is captured)
    q = update.callback_query
    await q.answer()
    parts = q.data.split("|")
    if len(parts) != 2:
        ...
        return
    username = parts[1]
    # store pending renew user in user_data so next message goes to recv_renew_days via ConversationHandler
...
def build_conv():
    # Important: include entry points for edit & renew-custom so conversation states work
...
def main():
    ...
    # action handler (renew/delete/edit menu)
    app.add_handler(CallbackQueryHandler(action_callback, pattern='^act_'))
    # renew/do & edit handlers (some are also in conv entry_points)
    ...
    # Conversation handler covers create flow, renew custom, edit flows
    app.add_handler(build_conv())
    logger.info("Public bot polling started (fixed v2)")
    app.run_polling()
...
echo "To test: open bot and send /start -> User List -> use Renew (Custom), Edit (change pw/expiry), Delete (card removed)"
