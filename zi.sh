#!/usr/bin/env bash
# install_zivpn_public_fixed_v6.3.sh
# Full fixed installer for ZIVPN + Telegram bot (v6.3)
# - User List shows cards with inline buttons
# - 3 days left shown in yellow color
# - Telegram notification on successful installation
...
echo "== ZIVPN + Public Telegram Bot Installer (fixed v6.3) =="
...
# Bot script (v6.3) - FIXED with User Cards in list and 3-day warning
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (fixed v6.3)
...
logger.info("Started zivpn public bot (fixed v6.3) admin_notify=%s", ADMIN_IDS)
...
# NEW: Helper to get all users for list
def get_all_users(limit=200):
    try:
        conn = sqlite3.connect(DB_PATH)
        cur = conn.cursor()
        cur.execute("SELECT username,password,created_at,expires_at,revoked FROM accounts WHERE revoked=0 ORDER BY id DESC LIMIT ?", (limit,))
        rows = cur.fetchall()
        conn.close()
        return rows
    except:
...
# NEW: Helper to create user card message
def create_user_card(username, password, expires_at, revoked):
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    body = (f"# ZI VPN BOT\n"
            ...
            f"{ram_line}\n"
            f"{disk_line}\n\n"
            f"Choose an option below (tap a button):")
...
async def handle_main_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == "Users List":
        ...
        for r in rows:
            ...
            body, status_emoji, left = create_user_card(username, password, expires_at, revoked)
        ...
        # Send bottom keyboard again
        kb = [
            [KeyboardButton("Create Account"), KeyboardButton("Users List"), KeyboardButton("Search")],
            [KeyboardButton("Help")]
        ]
        reply_kb = ReplyKeyboardMarkup(kb, resize_keyboard=True, one_time_keyboard=False)
        await update.message.reply_text("### Message\n\n88\n\n---", parse_mode=constants.ParseMode.MARKDOWN, reply_markup=reply_kb)
        return
    
    if txt == "Help":
        await update.message.reply_text("Public bot: use Create Account to add users. Use Users List / Search to manage.")
        return
...
async def handle_create_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ Create Account — Step 1:\nSend the username you want to add.")
...
async def global_text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt in ("Create Account", "Users List", "Search", "Help"):
        return
...
async def action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "act_delete":
        ...
        body, status_emoji, left = create_user_card(username, password, "", revoked)
        body = f"## Delete\n\n{body}"
        
        kb_confirm = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Confirm Delete", callback_data=f"confirm_delete|{username}"),
             InlineKeyboardButton("❌ Cancel", callback_data=f"cancel_delete|{username}")]
        ])
        
        await q.edit_message_text(body, parse_mode=constants.ParseMode.MARKDOWN, reply_markup=kb_confirm)
        return
...
async def delete_confirmation_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    data = q.data
    if "|" not in data:
    ...
    if action == "cancel_delete":
        await q.edit_message_text("❌ Delete cancelled.")
        return
    ...
    if action == "confirm_delete":
        ...
        try:
            ...
            await q.edit_message_text(f"✅ Account `{username}` has been deleted.\n\nTotal Users: `{total_users}`", parse_mode=constants.ParseMode.MARKDOWN)
            asyncio.create_task(notify_admins(context.application, f"🗑️ Account deleted:\nUsername: {username}\nTotal Users: {total_users}"))
        except Exception as e:
            await q.edit_message_text(f"❗ Failed to delete: {e}")
...
async def renew_do_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        body, status_emoji, left = create_user_card(username, password, new_exp.isoformat(), False)
        await q.edit_message_text(f"✅ `{username}` extended by {add_days} days.\n\n{body}", parse_mode=constants.ParseMode.MARKDOWN)
...
async def recv_renew_days(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Show updated user card
        body, status_emoji, left = create_user_card(username, password, new_exp.isoformat(), False)
        await update.message.reply_text(f"✅ `{username}` extended by {days} days.\n\n{body}", parse_mode=constants.ParseMode.MARKDOWN)
...
async def recv_edit_expiry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Show updated user card
        body, status_emoji, left = create_user_card(username, password, new_exp, False)
        await update.message.reply_text(f"✅ `{username}` expiry updated.\n\n{body}", parse_mode=constants.ParseMode.MARKDOWN)
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex('^(Users List|Help)$'), handle_main_buttons))
    ...
    app.add_handler(CallbackQueryHandler(delete_confirmation_callback, pattern='^(confirm_delete|cancel_delete)\\|'))
    ...
    logger.info("Public bot polling started (fixed v6.3)")
    app.run_polling()
...
# Send notification to admin after installation
echo "== INSTALL COMPLETE v6.3 =="
echo "✅ Fixed create account flow"
echo "✅ User List shows cards with inline buttons"
echo "✅ 3 days left shown in yellow color"
echo "✅ Telegram notification on successful installation"
echo ""

# Send notification to admin via Telegram
if [ -n "$BOT_TOKEN_DEFAULT" ] && [ -n "$ADMIN_IDS_DEFAULT" ]; then
    echo "Sending installation notification to admin..."
    ADMIN_ID=$(echo "$ADMIN_IDS_DEFAULT" | cut -d',' -f1)
    MESSAGE="Install script successful ✅ Script By : JueHtet"
    URL="https://api.telegram.org/bot${BOT_TOKEN_DEFAULT}/sendMessage"
    curl -s -X POST "$URL" \
        -d "chat_id=${ADMIN_ID}" \
        -d "text=${MESSAGE}" \
    ...
    echo "Note: BOT_TOKEN or ADMIN_IDS not set, skipping notification."
fi
...
echo ""
echo "To test:"
echo "1. Open bot and press /start"
echo "2. Tap 'Users List' button - should show user cards"
echo "3. Accounts with ≤3 days left will show in yellow"
echo "4. Try 'Create Account' flow"
echo ""
