...
# Fixed installer for ZIVPN + Telegram bot (v6.5)
...
# Bot script (v6.5) - UPDATED with NEW card format and FIXED deleted user recreation
...
def format_user_card(username: str, password: str, expires_at: str, revoked: int, index: int = 1):
    ...
    # NEW CARD FORMAT with icons - FIXED: Changed "Title VIP" to "Premium User"
    card = (
...
async def handle_main_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == "👥 Users List":
        await cmd_list_cards(update, context)  # Changed to show cards instead of text list
        return
    if txt == "ℹ️ Help":
...
async def cmd_list_cards(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for idx, row in enumerate(rows[:200], 1):  # Limit to 200 users, start index from 1
...
echo "3. All users displayed in NEW format with icons"
