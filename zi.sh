#!/usr/bin/env bash
# install_zivpn_public_fixed_v6.9.2.sh
# Enhanced installer for ZIVPN + Telegram bot (v6.9.2) with Button-Based Admin Commands
# - COMPLETELY FIXED: Restore function (file upload and restore process)
# - FIXED: Password addition to ZIVPN config
...
echo "== ZIVPN + Public Telegram Bot Installer (v6.9.2 - Fixed Restore) =="
...
# Restore script - COMPLETELY FIXED VERSION v6.9
sudo tee /usr/local/bin/zivpn_restore.py > /dev/null <<'PY'
#!/usr/bin/env python3
# ZIVPN Restore Script for Telegram Bot - COMPLETELY FIXED VERSION v6.9
...
# ============================================================================
# Bot script (v6.9.2) - COMPLETELY FIXED with Button-Based Interface
# RESTORE FUNCTION COMPLETELY FIXED
# ============================================================================
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (fixed v6.9.2) with BUTTON-BASED Interface
# COMPLETELY FIXED VERSION - Restore function completely fixed
...
logger.info("Started zivpn public bot (fixed v6.9.2 with Button Interface) admin_notify=%s", ADMIN_IDS)
if not BOT_TOKEN:
...
# ========== BACKUP/RESTORE FILE HANDLING - COMPLETELY FIXED VERSION ==========
async def handle_backup_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle backup file upload for restore - COMPLETELY FIXED VERSION"""
    logger.info(f"handle_backup_file called with message: {update.message}")
    ...
    if update.message.document:
        ...
        try:
            ...
            logger.info(f"File downloaded to: {file_path}, size: {os.path.getsize(file_path)} bytes")
            
            # Verify file exists and has content
            if not os.path.exists(file_path) or os.path.getsize(file_path) == 0:
                await msg.edit_text("❌ Downloaded file is empty or invalid.")
                shutil.rmtree(temp_dir, ignore_errors=True)
                return STATE_WAITING_BACKUP_FILE
            ...
            # Store file path and temp directory in context
            context.user_data['restore_file'] = file_path
            context.user_data['restore_temp_dir'] = temp_dir
            ...
            await msg.edit_text(f"❌ Error downloading file: {str(e)[:200]}")
            # Cleanup temp dir
            if 'temp_dir' in locals():
                shutil.rmtree(temp_dir, ignore_errors=True)
            return STATE_WAITING_BACKUP_FILE
    elif update.message.text and update.message.text.lower() in ['/cancel', 'cancel', '❌ cancel']:
        await cmd_cancel(update, context)
        return ConversationHandler.END
    else:
        await update.message.reply_text("❌ Please send a backup file (.tar.gz) or type /cancel to cancel")
        return STATE_WAITING_BACKUP_FILE
...
async def handle_restore_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle restore confirmation callback - COMPLETELY FIXED"""
    query = update.callback_query
    await query.answer()
    
    if query.data == "restore_no":
        # Cleanup temp file
        if 'restore_temp_dir' in context.user_data:
            temp_dir = context.user_data['restore_temp_dir']
            shutil.rmtree(temp_dir, ignore_errors=True)
            context.user_data.pop('restore_temp_dir', None)
        ...
        # Get bot token and admin ID for restore script
        bot_token = BOT_TOKEN
        ...
        def run_restore():
            try:
                logger.info(f"Starting restore process with file: {file_path}")
                
                # Run restore script with bot token and admin ID
                result = subprocess.run(
                    [RESTORE_SCRIPT, file_path, bot_token, str(admin_id)],
                    capture_output=True,
                    text=True,
                    timeout=300  # 5 minutes timeout for restore
                )
                
                logger.info(f"Restore script return code: {result.returncode}")
                logger.info(f"Restore script output: {result.stdout[:500] if result.stdout else 'No output'}")
                if result.stderr:
                    logger.error(f"Restore script stderr: {result.stderr[:500]}")
                ...
                try:
                    ...
                    data = {
                        'chat_id': admin_id,
                        'text': "❌ Restore timed out (took too long).",
                        'parse_mode': 'HTML'
                    }
                ...
                try:
                    ...
                    data = {
                        'chat_id': admin_id,
                        'text': f"❌ Restore error: {str(e)[:200]}",
                        'parse_mode': 'HTML'
                    }
                    requests.post(url, data=data, timeout=10)
...
# ========== CONVERSATION HANDLER BUILDER - FIXED ==========
def build_conv():
    conv = ConversationHandler(
        ...
        states={
            STATE_WAITING_BACKUP_FILE: [
                ...
                MessageHandler(filters.TEXT & filters.Regex('^(/cancel|Cancel|❌ Cancel|cancel)$', re.IGNORECASE), cmd_cancel),
...
def main():
    ...
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex('^(⚙️ Admin Panel)$'), handle_main_buttons))
    ...
    logger.info("Public bot polling started (fixed v6.9.2 with Button Interface)")
    app.run_polling()
...
echo ""
echo "== INSTALL COMPLETE v6.9.2 with BUTTON-BASED INTERFACE =="
echo ""
echo "✅✅✅ RESTORE FUNCTION COMPLETELY FIXED:"
echo "   - File upload now works properly"
echo "   - No more getting stuck at backup file request"
...
echo "   • 📥 Restore Backup - Restore from backup (NOW COMPLETELY FIXED)"
...
echo "   🔋 Status : 🟢 Online"
echo ""
echo "🔄 **RESTORE PROCESS (NOW WORKING):**"
echo "   1. Admin Panel → 📥 Restore Backup"
echo "   2. Send .tar.gz backup file"
echo "   3. Confirm restore"
echo "   4. System will restore and restart automatically"
echo "   5. Bot will send notification when restore is complete"
echo ""
...
echo ""
echo "✅ Installation complete with BUTTON-BASED INTERFACE and COMPLETELY FIXED RESTORE!"
