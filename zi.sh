...
# Bot script (v6.5) - UPDATED with DOMAIN in user cards
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (fixed v6.5) with DOMAIN support
...
logger.info("Started zivpn public bot (fixed v6.5 with DOMAIN) admin_notify=%s", ADMIN_IDS)
if not BOT_TOKEN:
...
def main():
    ...
    logger.info("Public bot polling started (fixed v6.5 with DOMAIN)")
    app.run_polling()
...
echo "== INSTALL COMPLETE v6.5 with DOMAIN =="
echo ""
...
echo "   👑 VIP crown"
echo "   📡 Server IP"
echo "   🌐 Server DNS (globe icon)"
echo "   👤 Username"
echo "   🔑 Password"
echo "   📅 Expiry date"
echo "   🟢/🔴/🟡 Status"
echo ""
echo "✅ Domain/DNS Features:"
echo "   - Domain is asked during installation"
echo "   - Shows in user cards with globe icon (🌐)"
echo "   - Shows in start command and help"
echo "   - Optional field - if not provided, won't show"
echo ""
...
echo "3. All users displayed in NEW format with domain (if provided)"
echo "4. Each card has Renew/Edit/Delete buttons"
echo "5. Success message shows ✅✅ Create Account Successfully 😄"
echo ""
