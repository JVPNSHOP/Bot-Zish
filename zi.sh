#!/usr/bin/env bash
# install_zivpn_public_fixed_v6.7.sh
# Fixed installer for ZIVPN + Telegram bot (v6.7) with COMPLETE Backup/Restore/Admin Panel
# - NEW User Card format with icons
# - FIXED: Restore function now properly restores passwords to config.json
# - FIXED: New accounts can connect after restore
# - ADDED: Complete Admin Panel with Backup/Restore buttons
# - ADDED: Auto-rebuild config.json from database during restore
# - ADDED: System status monitoring in Admin Panel
...
echo "== ZIVPN + Public Telegram Bot Installer (fixed v6.7) =="
...
# Restore script - FIXED VERSION with password rebuilding
sudo tee /usr/local/bin/zivpn_restore.py > /dev/null <<'PY'
#!/usr/bin/env python3
# ZIVPN Restore Script for Telegram Bot - FIXED VERSION with password rebuilding
...
def rebuild_config_from_database():
    """Rebuild config.json from database - FIXED VERSION"""
    print("Rebuilding config.json from database...")
    
    db_path = "/var/lib/zivpn_bot/accounts.db"
    config_path = "/etc/zivpn/config.json"
    
    if not os.path.exists(db_path):
        print("Database not found, skipping config rebuild")
        return False
    ...
    try:
        ...
        # Get all active passwords from database
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT password FROM accounts WHERE revoked=0")
        active_passwords = [row[0] for row in cursor.fetchall()]
        conn.close()
        
        print(f"Found {len(active_passwords)} active passwords in database")
        
        # Update config with passwords
        if 'auth' not in config:
            config['auth'] = {'mode': 'passwords', 'config': []}
        
        # Merge passwords, remove duplicates
        existing_passwords = set(config['auth'].get('config', []))
        all_passwords = list(set(active_passwords) | existing_passwords)
        
        config['auth']['config'] = all_passwords
        config['obfs'] = ""  # Ensure obfs is disabled
        
        # Write back config
        with open(config_path, 'w') as f:
            json.dump(config, f, indent=2)
        
        print(f"Config rebuilt with {len(all_passwords)} passwords")
        return True
        
    except Exception as e:
        print(f"Error rebuilding config: {e}")
        return False
...
def restore_backup(backup_file_path, bot_token=None, admin_id=None):
    ...
    try:
        ...
        # 5. REBUILD CONFIG FROM DATABASE - THIS IS THE FIX
        print("Rebuilding config.json from database...")
        if rebuild_config_from_database():
            print("  ✓ Config rebuilt from database")
        else:
            print("  ⚠️ Config rebuild failed, but continuing...")
        
        # 6. Send progress notification
        if bot_token and admin_id:
            send_telegram_message(bot_token, admin_id, "✅ Data restored, rebuilding config and restarting services...")
        
        # 7. Start zivpn service
...
# Bot script (v6.7) - FIXED with complete Admin Panel and password rebuilding
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (fixed v6.7) with COMPLETE Admin Panel
...
logger.info("Started zivpn public bot (fixed v6.7 with Complete Admin Panel) admin_notify=%s", ADMIN_IDS)
if not BOT_TOKEN:
...
async def show_admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show complete Admin Panel with buttons"""
    keyboard = [
        [InlineKeyboardButton("📦 Backup System", callback_data="admin_backup"),
         InlineKeyboardButton("📥 Restore System", callback_data="admin_restore")],
        [InlineKeyboardButton("📊 System Stats", callback_data="admin_stats"),
         InlineKeyboardButton("👥 All Users", callback_data="admin_listall")],
        [InlineKeyboardButton("🔄 Restart Services", callback_data="admin_restart"),
         InlineKeyboardButton("🗑️ Uninstall", callback_data="admin_uninstall")],
    ...
    deleted_users = cur.fetchone()[0] or 0
    conn.close()
    ...
    await update.message.reply_text(admin_text, parse_mode=constants.ParseMode.MARKDOWN, reply_markup=reply_markup)
...
async def admin_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "admin_back":
        ...
        # Show admin panel again after backup
        await asyncio.sleep(2)
        await show_admin_panel(update, context)
    
    elif query.data == "admin_restore":
        await query.edit_message_text("📥 Starting restore process...")
        await cmd_restore(update, context)
    
    elif query.data == "admin_stats":
        await query.edit_message_text("📊 Getting system statistics...")
        await cmd_stats(update, context)
        # Show admin panel again after stats
        await asyncio.sleep(2)
        await show_admin_panel(update, context)
    
    elif query.data == "admin_listall":
        await query.edit_message_text("👥 Getting user list...")
        await cmd_listall(update, context)
        # Show admin panel again after list
        await asyncio.sleep(2)
        await show_admin_panel(update, context)
        ...
        # Show admin panel again after restart
        await asyncio.sleep(2)
        await show_admin_panel(update, context)
    
    elif query.data == "admin_uninstall":
        await query.edit_message_text("🗑️ Starting uninstall process...")
...
async def handle_main_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == "ℹ️ Help":
        ...
        if update.effective_user.id in ADMIN_IDS:
            help_text += "\n👑 Admin Commands:\n"
            help_text += "/admin - Show admin panel\n"
        ...
        return
    if txt == "👑 Admin Panel" and update.effective_user.id in ADMIN_IDS:
        await show_admin_panel(update, context)
        return
...
async def global_text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt in ("🆕 Create Account", "👥 Users List", "🔍 Search", "ℹ️ Help", "👑 Admin Panel"):
        return
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex('^(👥 Users List|ℹ️ Help|👑 Admin Panel)$'), handle_main_buttons))
    ...
    logger.info("Public bot polling started (fixed v6.7 with Complete Admin Panel)")
    app.run_polling()
...
echo ""
echo "== INSTALL COMPLETE v6.7 with COMPLETE Admin Panel and FIXED Restore =="
echo ""
echo "✅ **MAJOR FIXES IN v6.7:**"
echo "   1. Restore process now REBUILDS config.json from database"
echo "   2. All user passwords automatically added to config.json after restore"
echo "   3. New accounts can connect immediately after restore"
echo "   4. Complete Admin Panel with all controls"
echo ""
echo "👑 **COMPLETE ADMIN PANEL FEATURES:**"
echo "   📦 Backup System - Create and download backup"
echo "   📥 Restore System - Upload and restore backup (FIXED)"
echo "   📊 System Stats - Real-time server monitoring"
echo "   👥 All Users - View all users including deleted"
echo "   🔄 Restart Services - Restart ZIVPN and Bot"
echo "   🗑️ Uninstall - Complete system uninstall"
echo ""
echo "🔧 **Restore Process FIXED:**"
echo "   1. Upload backup file via Telegram"
echo "   2. Restore database, config, and settings"
echo "   3. AUTO-REBUILD config.json from database passwords"
echo "   4. Restart services with all users active"
echo "   5. New accounts work immediately"
echo ""
echo "📋 **Migration Workflow (Server to Server):**"
echo "1. On old server: Use /admin → 📦 Backup System"
...
echo "5. Use /admin → 📥 Restore System and upload backup file"
echo "6. ✅ All users and passwords automatically restored and working!"
echo ""
...
echo "3. Send /admin or click '👑 Admin Panel' button"
echo ""
echo "To check bot logs: sudo journalctl -u zivpn_bot.service -n 200 --no-pager"
echo "✅ Installation complete with FIXED Restore and Complete Admin Panel!"
