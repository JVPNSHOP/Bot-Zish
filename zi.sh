...
# - User Card now has Renew, Edit, Delete
# - Edit supports changing password (or /gen) and changing expiry days
# - Status emoji: 🟢 (online), 🟡 (<=3 days), 🔴 (expired)
set -euo pipefail
IFS=$'\n\t'
...
echo "== ZIVPN + Public Telegram Bot Installer (fixed, user edit/renew/delete) =="
...
# Bot script (public, user cards: Renew / Edit / Delete; edit can change password or expiry)
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (user edit/renew/delete)
...
def main():
    ...
    # allow other callback patterns handled in conv entry points
