...
# - Status uses emoji: 🟢 (online), 🟡 (expiring, <=3 days), 🔴 (expired)
# - Blink implemented by editing message (emoji on/off)
# - CPU/RAM/STORAGE shown only in /start menu, not in user cards
set -euo pipefail
IFS=$'\n\t'
...
echo "== ZIVPN + Public Telegram Bot Installer (fixed, server stats in menu only) =="
...
# Bot script (public, server stats in /start only; user cards include Renew/Delete; status uses colored emojis)
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (server stats in menu only; colored emoji status + blink)
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "menu_list":
        ...
        for r in rows[:100]:
            ...
            # User Card WITHOUT CPU/RAM/STORAGE
