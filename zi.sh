#!/usr/bin/env bash
# install_zivpn_with_admin.sh
# Full installer: installs ZIVPN UDP, helper scripts, sqlite DB and Telegram bot (menu + UI, renew/delete)
# Embedded BOT_TOKEN and ADMIN_IDS as requested.
set -euo pipefail
IFS=$'\n\t'

### ====== EMBEDDED CREDENTIALS (from user) ======
BOT_TOKEN_EMBED="8409119937:AAGsRoqiyg-U_caPKsDUpnhzMN8SyG5Y0qw"
ADMIN_IDS_EMBED="14944445684"
### ==============================================

# Auto-detect public IP (fallback string if fails)
ZIVPN_SERVER_IP_DETECT="$(curl -s https://ifconfig.co || echo "your.server.ip")"

echo "=== ZIVPN + Telegram Bot Installer (with embedded admin/token) ==="
...
if ! sudo wget -q https://raw.githubusercontent.com/zahidbd2/udp-zivpn/main/config.json -O /etc/zivpn/config.json; then
  echo "-- Creating fallback /etc/zivpn/config.json --"
...
# 4) Helper script to create accounts
...
# 5) Python env and install bot deps
...
# 7) Write environment file (with embedded token/admin)
echo "-- Writing /etc/default/zivpn_bot (embedded token & admin) --"
sudo tee /etc/default/zivpn_bot > /dev/null <<EOF
# BOT environment for zivpn_bot
BOT_TOKEN="${BOT_TOKEN_EMBED}"
ADMIN_IDS="${ADMIN_IDS_EMBED}"
ZIVPN_SERVER_IP="${ZIVPN_SERVER_IP_DETECT}"
EOF

sudo chmod 644 /etc/default/zivpn_bot
sudo chown root:root /etc/default/zivpn_bot

# 8) Write bot script (menu + create + user cards + renew/delete + /create CLI command)
echo "-- Writing bot script to /opt/zivpn_bot/bot.py --"
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - ZIVPN Telegram bot (menu, create wizard, user cards with renew/delete)
import os, sqlite3, subprocess, datetime, json, logging
...
for x in ADMIN_IDS_RAW.split(","):
    ...
    if x:
        try:
            ...
            logger.warning("invalid admin id skipped: %s", x)
...
def run_create(username:str, days:int, pwd:str=None):
...
async def menu_cb(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if q.data=="menu_create":
        if not is_admin(user.id):
            ...
            return
        await q.edit_message_text("Step 1: Send username to create:", parse_mode=constants.ParseMode.MARKDOWN)
    ...
    if q.data=="menu_help":
        await q.edit_message_text("Use Create Account to add users. Only admins can manage accounts.", parse_mode=constants.ParseMode.MARKDOWN)
        return
...
async def recv_username(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Step 2: Send password or /gen to auto-generate.")
...
async def expiry_cb(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    kb=[[InlineKeyboardButton("Add Account ✅", callback_data="confirm_add"),
...
async def recv_custom_days(update:Update, context:ContextTypes.DEFAULT_TYPE):
    txt=update.message.text.strip()
    if not txt.isdigit():
        await update.message.reply_text("Send numeric days.")
        return STATE_EXPIRY
    days=int(txt)
    context.user_data['new_days']=days
    expiry_date=(datetime.date.today()+datetime.timedelta(days=days)).isoformat()
...
async def confirm_cb(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if q.data=="confirm_add":
        ...
        try:
            pw, exp = run_create(username, days, password)
            ...
            cur.execute("INSERT INTO accounts (username,password,created_by,created_at,expires_at) VALUES (?,?,?,?,?)",
                        (username, pw, user.id, datetime.datetime.utcnow().isoformat(), exp))
            conn.commit(); conn.close()
            await q.edit_message_text(f"✅ Created\nUsername: `{username}`\nPassword: `{pw}`\nExpires: `{exp}`", parse_mode=constants.ParseMode.MARKDOWN)
        except sqlite3.IntegrityError:
            await q.edit_message_text("Username exists. Revoke first or choose another.")
        except Exception as e:
...
async def action_cb(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if act=="act_delete":
        ...
        try:
            ...
            conn.commit(); conn.close()
            await q.edit_message_text(f"User `{username}` revoked.", parse_mode=constants.ParseMode.MARKDOWN)
        except Exception as e:
            await q.edit_message_text(f"Failed to revoke: {e}")
            conn.close()
    elif act=="act_renew":
        kb=[[InlineKeyboardButton("➕5 days", callback_data=f"renew_do|{username}|5"),
            ...
            [InlineKeyboardButton("Custom", callback_data=f"renew_custom|{username}")]]
        await q.edit_message_text(f"Renew `{username}` — choose extension:", reply_markup=InlineKeyboardMarkup(kb))

async def renew_do_cb(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await q.edit_message_text(f"Failed to renew: {e}"); conn.close()

async def renew_custom_cb(update:Update, context:ContextTypes.DEFAULT_TYPE):
    q=update.callback_query; await q.answer()
    data=q.data; user=q.from_user
    if not is_admin(user.id):
        await q.edit_message_text("Unauthorized."); return
    ...
    await q.edit_message_text(f"Send number of days to extend `{username}` by (e.g., 30).")
...
async def recv_renew_days(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if 'pending_renew_user' not in context.user_data:
        await update.message.reply_text("No pending renew."); return ConversationHandler.END
    txt=update.message.text.strip()
    if not txt.isdigit():
        await update.message.reply_text("Send numeric days."); return STATE_RENEW
    days=int(txt); username=context.user_data.pop('pending_renew_user')
    conn=sqlite3.connect(DB_PATH); cur=conn.cursor()
    cur.execute("SELECT expires_at FROM accounts WHERE username=? LIMIT 1",(username,))
    row=cur.fetchone()
    if not row:
        await update.message.reply_text("User not found."); conn.close(); return ConversationHandler.END
    expires_at=row[0]
    try:
        curr = datetime.datetime.fromisoformat(expires_at).date() if expires_at else datetime.date.today()
        ...
        await update.message.reply_text(f"Failed to renew: {e}"); conn.close()
    return ConversationHandler.END
...
# CLI /create command for admins
async def cmd_create(update:Update, context:ContextTypes.DEFAULT_TYPE):
    user=update.effective_user
    if not is_admin(user.id):
        await update.message.reply_text("Unauthorized."); return
    if len(context.args) < 2:
        await update.message.reply_text("Usage: /create <username> <days> [password]"); return
    username=context.args[0]
    try:
        days=int(context.args[1])
    except:
        await update.message.reply_text("Days must be integer."); return
    pwd=context.args[2] if len(context.args)>=3 else None
    try:
        pw, exp = run_create(username, days, pwd)
        if not pw:
            pw = pwd or "unknown"
        if not exp:
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(action_cb, pattern='^act_'))
    app.add_handler(CallbackQueryHandler(renew_do_cb, pattern='^renew_do\\|'))
    app.add_handler(CallbackQueryHandler(renew_custom_cb, pattern='^renew_custom\\|'))
    app.add_handler(CallbackQueryHandler(confirm_cb, pattern='^confirm_'))
...
# 9) systemd unit for bot (with EnvironmentFile)
...
echo "Important:"
echo " - /etc/default/zivpn_bot contains the embedded BOT_TOKEN and ADMIN_IDS you asked to add."
echo " - If you change /etc/default/zivpn_bot, run:"
echo "     sudo systemctl daemon-reload"
echo "     sudo systemctl restart zivpn_bot.service"
echo ""
echo "To test:"
echo " - In Telegram: send /start to your bot, press Create Account (admin) or use /create <username> <days> [password]"
echo " - Check logs: sudo journalctl -u zivpn_bot.service -f"
echo ""
echo "Security reminder: Rotate your bot token if it was publicly exposed."
echo "Done."
