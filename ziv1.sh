...
# - Ensures passwords are stored in auth.config (auth -> config)
# - Restarts zivpn.service after changes
# - Adds Total Users count in bot messages
set -euo pipefail
IFS=$'\n\t'
...
echo "== ZIVPN + Public Telegram Bot Installer (fixed, auth.config) =="
...
if 'config' in j and not ( 'auth' in j and isinstance(j['auth'],dict) and isinstance(j['auth'].get('config',None),list) and j['auth'].get('config')!=j.get('config') ):
    # move root-level config entries into auth.config
    root_cfg=j.get('config',[])
    if isinstance(root_cfg,list):
        for p in root_cfg:
            if p not in j['auth'].get('config',[]):
                j['auth'].setdefault('config',[]).append(p)
        changed=True
    # remove root-level config after moving
    if 'config' in j:
        del j['config']
        changed=True
# ensure auth.config is list
if 'auth' in j and isinstance(j['auth'],dict):
    if 'config' not in j['auth'] or not isinstance(j['auth']['config'],list):
        j['auth']['config'] = j['auth'].get('config',[]) if isinstance(j['auth'].get('config',[]),list) else []
        changed=True
if changed:
...
# Bot script (public, shows total users; unchanged behavior except count added)
