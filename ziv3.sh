#!/usr/bin/env bash
# install_zivpn_public_fixed_v5.sh
# Full fixed installer for ZIVPN + Telegram bot (v5)
# - Fixes Edit/Renew handlers typo & keeps obfs disabled by default
set -euo pipefail
IFS=$'\n\t'
...
echo "== ZIVPN + Public Telegram Bot Installer (fixed v5) =="
...
# Bot script (v5) - fixes typo in edit flow & keeps consistent handlers
sudo tee /opt/zivpn_bot/bot.py > /dev/null <<'PY'
#!/usr/bin/env python3
# bot.py - Public ZIVPN bot (fixed v5)
...
logger.info("Started zivpn public bot (fixed v5) admin_notify=%s", ADMIN_IDS)
...
async def action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "act_delete":
        ...
        # FIXED: use edit_message_text (was reply_text typo previously)
...
def main():
    ...
    logger.info("Public bot polling started (fixed v5)")
    app.run_polling()
...
echo "== INSTALL COMPLETE v5 =="
echo "Fixed Edit/Renew typo; obfs disabled. If you skipped BOT_TOKEN during install, edit /etc/default/zivpn_bot and set BOT_TOKEN=\"<your token>\" then: sudo systemctl restart zivpn_bot.service"
echo "If Edit/Renew still fail, send: sudo journalctl -u zivpn_bot.service -n 200 --no-pager output and I'll inspect."
echo "Done."
